...
namespace dbSetup
{
    public class DataGenerator
    {
        ...
        public static void CreateIndices(string connectionStr)
        {
            using (NpgsqlConnection connection = new NpgsqlConnection(connectionStr))
            {
                connection.Open();
                CreateIndex("PERSON", new string[] { "nickname" }, connection);
                CreateIndex("PERSON_CHAT", new string[] { "person_id", "chat_id" }, connection);
                CreateIndex("CHAT", new string[] { "name" }, connection);
                CreateIndex("MESSAGE", new string[] { "created_at" }, connection);
                CreateIndex("DOCUMENT", new string[] { "filename" }, connection);
                CreateIndex("INDUSTRY", new string[] { "industry_name", }, connection);
                CreateIndex("PROJECT_INDUSTRY", new string[] { "project_id", "industry_id" }, connection);
                CreateIndex("PROJECT", new string[] { "name" }, connection);
                CreateIndex("TOPIC", new string[] { "name" }, connection);
                CreateIndex("TIMEPOINT", new string[] { "date" }, connection);
            }
        }

        private static void CreateIndex(string table, string[] columns, NpgsqlConnection connection) 
        {
            using (NpgsqlCommand cmd = new NpgsqlCommand()) 
            {
                cmd.Connection = connection;
                string colsStr = string.Join(", ", columns);
                string command = $"CREATE INDEX idx_{table} ON {table}({colsStr});";
